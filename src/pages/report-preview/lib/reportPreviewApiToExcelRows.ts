...
import { roundDecimalHoursToMinute } from '@shared/lib/formatTrackingHours';
import type {
...
function buildTimeExcelRow(params: {
    ...
    const bhFromApi = deriveBillableHoursForEntry(it, rollup) ?? 0;
    ...
    const rate = rateFromApi > 0
        ? rateFromApi
        : bhFromApi > 0
            ? Math.round((ba / bhFromApi) * 10000) / 10000
            : 0;
    const bhResolved = resolveReportBillableHours(bhFromApi, ba, rate);
    const tri = entryBillableTriState(it);
    const isBillable = tri === true || (tri === null && bhResolved > 1e-6);
    const rawHours = Number.isFinite(it.hours) ? it.hours : 0;
    const hours = roundDecimalHoursToMinute(rawHours);
    let billableHours = roundDecimalHoursToMinute(bhResolved);
    if (rowKind === 'entry' && isBillable && Math.abs(hours - billableHours) * 60 < 1)
        billableHours = hours;
    ...
    const noteRaw = str(it.notes) || str(it.description);
    const note = resolveTimeEntryNotesOnly(noteRaw, taskTitle);
    ...
    const entryInitials = entryString(it, ['employee_initials', 'employeeInitials']);
    const userInitials = str(u.initials);
    const explicitInitials = entryInitials || userInitials;
    const employeeInitials = explicitInitials
        ? resolveReportEmployeeInitials({ stored: explicitInitials, displayName: empName })
        : '';
...
export function flattenTimeReportToExcelRows(groupBy: TimeGroup, rows: TimeReportRow[]): TimeExcelPreviewRow[] {
    const out: TimeExcelPreviewRow[] = [];
    if (groupBy === 'team') {
        for (const r of rows as TimeRowTeam[]) {
            const cur = str(r.currency) || 'USD';
            out.push({
                rowKey: `team-${r.user_id}-${cur}`,
                timeEntryId: '',
                rowKind: 'aggregate',
                sourceEntryCount: 0,
                ...
                employeeInitials: str(r.initials)
                    ? resolveReportEmployeeInitials({ stored: str(r.initials), displayName: r.user_name })
                    : '',
                employeePosition: r.is_contractor ? 'Подрядчик' : '',
                workDate: '',
                recordedAt: '',
                clientId: '',
                clientName: '',
                ...
                isBillable: r.billable_hours > 0,
                taskBillableByDefault: false,
                isInvoiced: false,
                isPaid: false,
                isWeekSubmitted: false,
                billableRate: 0,
                amountToPay: r.billable_amount,
                costRate: 0,
                costAmount: 0,
                currency: cur,
    ...
    for (const row of rows) {
        const users = 'users' in row ? row.users ?? [] : [];
        let parentClientId = '';
        ...
        if (groupBy === 'projects') {
            ...
            parentClientId = str(r.client_id);
            parentClientName = str(r.client_name);
            parentProjectId = str(r.project_id);
            parentProjectName = str(r.project_name);
        }
