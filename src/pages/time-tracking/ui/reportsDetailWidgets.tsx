...
export function TimeEntryLogDetails({ entries, entriesTotal, entriesTruncated, currency: defaultCurrency = '', entryGroupContext, userBillableRollup, groupBy, }: {
    ...
    <div className="rp2-entries__caption" aria-label={t('timeTrackingPage.reports.entryLog.captionAria').replace('{count}', String(n))}>
      <span className="rp2-entries__caption-label">{t('timeTrackingPage.reports.entryLog.caption')}</span>
      <span className="rp2-entries__summary-count">{n}</span>
    </div>
    <div className={colClass}>
      <div className="rp2-entries__head" role="row">
        <div role="columnheader">{t('timeTrackingPage.reports.entryLog.date')}</div>
        <div role="columnheader">{t('timeTrackingPage.reports.entryLog.time')}</div>
        <div className="rp2-num" role="columnheader">{t('timeTrackingPage.reports.entryLog.hours')}</div>
        <div className="rp2-num" role="columnheader">{t('timeTrackingPage.reports.entryLog.billableHours')}</div>
        <div className="rp2-num" role="columnheader">{t('timeTrackingPage.reports.entryLog.amount')}</div>
        <div role="columnheader">{t('timeTrackingPage.reports.entryLog.status')}</div>
        {showProject && <div role="columnheader">{t('timeTrackingPage.reports.entryLog.project')}</div>}
        {showClient && <div role="columnheader">{t('timeTrackingPage.reports.entryLog.client')}</div>}
        {showTask && <div role="columnheader">{t('timeTrackingPage.reports.entryLog.task')}</div>}
        <div role="columnheader">{t('timeTrackingPage.reports.entryLog.comment')}</div>
      </div>
      {entries.map((it, idx) => {
          ...
          <div className="rp2-entries__cell">
            <span className={billableChipClass(billKind)}>{billLabel}</span>
          </div>
    ...
    {entriesTruncated && entriesTotal != null && entriesTotal > entries.length ? (<p className="rp2-entries__note">
      {t('timeTrackingPage.reports.entryLog.truncated')
            .replace('{shown}', String(entries.length))
            .replace('{total}', String(entriesTotal))}
    </p>) : null}
  </div>);
}
...
export function PctBar({ a, b, }: {
    ...
    return (<div className={`rp2-pct rp2-pct--${tone}`} title={percent != null ? `${percent}%` : t('timeTrackingPage.reports.entryLog.noData')}>
