...
function darkChromeFromPhotoHue(avgR: number, avgG: number, avgB: number, lightness: number, alpha: number): string {
...
export function createDefaultTodoThemeVars(accent = '#1d4ed8'): ThemeVars {
...
function isBrowserSameOriginAssetUrl(url: string): boolean {
...
async function loadImageElement(src: string): Promise<HTMLImageElement> {
    const img = new Image();
    img.decoding = 'async';
    if (!src.startsWith('blob:') && !isBrowserSameOriginAssetUrl(src))
...
async function resolveThemeAnalysisImageUrl(imageUrl: string): Promise<{
    src: string;
    revoke?: () => void;
}> {
    if (imageUrl.startsWith('blob:') || isBrowserSameOriginAssetUrl(imageUrl))
        return { src: imageUrl };
    const res = await fetch(imageUrl, { credentials: 'include', mode: 'cors' });
    if (!res.ok)
...
export function deriveColumnColors(accentHex: string): Pick<ThemeVars, 'columnTodayBg' | 'columnTodayText' | 'columnWeekBg' | 'columnWeekText' | 'columnLaterBg' | 'columnLaterText'> {
...
export async function deriveThemeFromImage(imageUrl: string): Promise<ThemeVars> {
    const { src, revoke } = await resolveThemeAnalysisImageUrl(imageUrl);
    try {
        const img = await loadImageElement(src);
        return deriveThemeFromLoadedImage(img);
    }
    finally {
        revoke?.();
    }
...
function deriveThemeFromLoadedImage(img: HTMLImageElement): ThemeVars {
