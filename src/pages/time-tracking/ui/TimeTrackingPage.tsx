...
function EmployeeScopePicker({ currentUser, ttScopeUsers, selectedId, onSelect }: EmployeeScopePickerProps) {
    ...
    return (<div className="tt-scope-picker" ref={wrapRef}>
      ...
      {open && (<div className="tt-scope-panel" role="listbox" aria-label={t('timeTrackingPage.page.scopePickerAria')}>
          <div className="tt-scope-panel__header">
            <span className="tt-scope-panel__title">{t('timeTrackingPage.page.scopeEmployee')}</span>
          </div>
          {options.length > 5 && (<div className="tt-scope-panel__search">
              ...
              <input ref={inputRef} className="tt-scope-panel__search-input" placeholder={t('timeTrackingPage.page.scopeSearch')} value={query} onChange={(e) => setQuery(e.target.value)}/>
          ...
          <ul className="tt-scope-panel__list">
            {filtered.length === 0 && (<li className="tt-scope-panel__empty">{t('timeTrackingPage.page.scopeEmpty')}</li>)}
            {filtered.map((u) => {
...
export function TimeTrackingPage() {
    ...
    function handleTabChangeFromSidebar(id: TimeTabId) {
        handleTabChange(id);
    ...
    return (<div className="time-page time-page--enter">
      <main className="time-page__main">
        {accessDenied && (<div className="time-page__dev-overlay" role="status" aria-label={t('timeTrackingPage.page.accessDeniedAria')}>
            <div className="time-page__dev-overlay-inner">
              ...
              <p className="time-page__dev-overlay-text">
                {t('timeTrackingPage.page.accessDeniedText')}
              </p>
              <AppBackButton to={routes.home} />
            </div>
          </div>)}
        
        <nav className="time-page__navbar" aria-label={t('timeTrackingPage.page.navAria')}>
          <AppBackButton to={routes.home} hideLabelOnMobile />
          <button
            type="button"
            className="time-page__navbar-menu-btn"
            onClick={() => setMobileNavOpen(true)}
            aria-label={t('timeTrackingPage.page.openMenu')}
            aria-expanded={mobileNavOpen}
            aria-controls="time-page-mobile-nav"
          >
            <IconMenu />
          </button>
          <div className="time-page__navbar-sep time-page__navbar-sep--logo" aria-hidden="true"/>
          <button
            type="button"
            className="time-page__navbar-logo-btn"
            onClick={() => navigate(routes.home)}
            aria-label={t('brand.homeAria')}
            title={t('brand.homeAria')}
          >
            <img src="/logo.svg" alt="" className="time-page__navbar-logo-img" width={26} height={38} draggable={false}/>
          </button>
          {loading ? (<>
              ...
              <div className="time-page__navbar-tabs time-page__navbar-tabs--skel" aria-hidden="true">
                {[118, 60, 56, 48, 72, 58, 66, 80, 64].map((w, i) => (<span key={i} className="time-page__navbar-tab-skel" style={{ width: w }}/>))}
              </div>
            </>) : hasAccess && user ? (<>
              <div className="time-page__navbar-sep time-page__navbar-sep--tabs" aria-hidden="true"/>
              <span className="time-page__navbar-current" title={activeTabLabel}>{activeTabLabel}</span>
              <div className="time-page__navbar-tabs" role="tablist" aria-label={t('timeTrackingPage.page.sectionsTablistAria')}>
                {visibleTabDefs.map((tab) => (<button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`time-tab-${tab.id}`} id={`time-tab-btn-${tab.id}`} className={`time-page__navbar-tab${activeTab === tab.id ? ' time-page__navbar-tab--active' : ''}`} onClick={() => handleTabChange(tab.id)} aria-label={tab.id === 'reports' && forReviewBadgeAria ? `${tab.label}. ${forReviewBadgeAria}` : undefined}>
                    <span className="time-page__navbar-tab-inner">
                      {tab.label}
                      {tab.id === 'reports' && forReviewBadge ? (<span className="time-page__navbar-tab-badge" aria-hidden>{forReviewBadge}</span>) : null}
                    </span>
                  </button>))}
              </div>
            </>) : null}
        ...
        {typeof document !== 'undefined' && hasAccess && user ? createPortal(<>
            <button
              type="button"
              className={`time-page__mobile-nav-backdrop${mobileNavOpen ? ' time-page__mobile-nav-backdrop--visible' : ''}`}
              aria-hidden={!mobileNavOpen}
              tabIndex={mobileNavOpen ? 0 : -1}
              onClick={() => setMobileNavOpen(false)}
            />
            <aside
              id="time-page-mobile-nav"
              className={`time-page__mobile-nav${mobileNavOpen ? ' time-page__mobile-nav--open' : ''}`}
              aria-hidden={!mobileNavOpen}
              aria-label={t('timeTrackingPage.page.sidebarAria')}
            >
              <div className="time-page__mobile-nav-head">
                <div className="time-page__mobile-nav-brand">
                  <img src="/logo.svg" alt="" className="time-page__mobile-nav-logo" width={22} height={32} draggable={false}/>
                  <span className="time-page__mobile-nav-title">{t('timeTrackingPage.page.title')}</span>
                </div>
                <button
                  type="button"
                  className="time-page__mobile-nav-close"
                  onClick={() => setMobileNavOpen(false)}
                  aria-label={t('timeTrackingPage.page.closeMenu')}
                />
              </div>
              <nav className="time-page__mobile-nav-list" role="tablist" aria-label={t('timeTrackingPage.page.sectionsTablistAria')}>
                {visibleTabDefs.map((tab) => (<button
                    key={tab.id}
                    type="button"
                    role="tab"
                    aria-selected={activeTab === tab.id}
                    aria-controls={`time-tab-${tab.id}`}
                    className={`time-page__mobile-nav-item${activeTab === tab.id ? ' time-page__mobile-nav-item--active' : ''}`}
                    onClick={() => handleTabChangeFromSidebar(tab.id)}
                    aria-label={tab.id === 'reports' && forReviewBadgeAria ? `${tab.label}. ${forReviewBadgeAria}` : undefined}
                  >
                    <span className="time-page__mobile-nav-item-label">{tab.label}</span>
                    {tab.id === 'reports' && forReviewBadge ? (<span className="time-page__mobile-nav-item-badge" aria-hidden>{forReviewBadge}</span>) : null}
                  </button>))}
              </nav>
              {headerManagerTrailing ? (<div className="time-page__mobile-nav-foot">
                  {headerManagerTrailing}
                </div>) : null}
            </aside>
          </>, document.body) : null}
        ...
        {hasAccess && user && (<>
                ...
                {activeTab === 'reports' && canViewTimeTrackingReports(user) && (<div className="time-page__content time-page__content--enter" role="tabpanel" id="time-tab-reports" aria-labelledby="time-tab-btn-reports">
                    <TimeTrackingPanelSuspense>
                      <ReportsPanel />
                    </TimeTrackingPanelSuspense>
                  </div>)}
                {activeTab === 'statistics' && (<div className="time-page__content time-page__content--enter" role="tabpanel" id="time-tab-statistics" aria-labelledby="time-tab-btn-statistics">
                    <TimeTrackingPanelSuspense>
                      <StatisticsPanel />
                    </TimeTrackingPanelSuspense>
                  </div>)}
