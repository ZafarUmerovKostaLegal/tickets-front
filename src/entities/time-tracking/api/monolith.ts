...
import { isActiveTimeManagerClientRow, isActiveTimeManagerProjectRow } from '../lib/projectTimeEntry';
import {
    reportCacheGet,
    reportCacheSet,
    reportCacheInvalidateAll as _invalidateReportCache,
...
async function fetchTimeTrackingUsersImpl(): Promise<TimeTrackingUserRow[]> {
    const res = await apiFetch('/api/v1/time-tracking/users');
    await throwIfNotOk(res);
...
function fetchTimeTrackingUsersCached(): Promise<TimeTrackingUserRow[]> {
...
function normalizeTeamWorkloadMember(raw: unknown): TeamWorkloadMember | null {
    if (raw == null || typeof raw !== 'object')
        return null;
    const o = raw as Record<string, unknown>;
...
function normalizeTeamWorkloadResponse(raw: unknown): TeamWorkloadResponse {
...
const userProjectAccessInflight = new Map<number, Promise<UserProjectAccessOut>>();
const projectAccessPickInflight = new Map<string, Promise<ProjectPartnerAccessRow[]>>();
...
async function fetchUserProjectAccess(authUserId: number): Promise<UserProjectAccessOut> {
...
async function listProjectAccessPickRows(
    projectId: string,
...
async function findTimeManagerClientProjectById(projectId: string): Promise<TimeManagerClientProjectRow | null> {
...
function readProjectPartnerAuthUserIdsFromRow(p: TimeManagerClientProjectRow): number[] {
    const raw = p.partnerAuthUserIds ?? p.partner_auth_user_ids ?? [];
    return raw.map((id) => Number(id)).filter((id) => Number.isFinite(id) && id > 0);
}

function readProjectTeamAuthUserIdsFromRow(p: TimeManagerClientProjectRow): number[] {
...
function projectPartnerAccessRowsFromAuthUserIds(
...
function mergeProjectPartnerAccessRows(
...
async function listProjectTeamMembersFromProjectDefinition(
    projectId: string,
...
async function listProjectPartnersFromProjectDefinition(
    projectId: string,
...
function normalizeTimeTrackingTeamMemberPreview(raw: unknown): TimeTrackingTeamMemberPreview | null {
    if (raw == null || typeof raw !== 'object')
        return null;
    const o = raw as Record<string, unknown>;
...
function parseTimeTrackingTeamsResponse(raw: unknown): TimeTrackingTeamRow[] {
    let arr: unknown[] = [];
    if (Array.isArray(raw))
        arr = raw;
    else if (raw && typeof raw === 'object') {
...
function normalizeLaborMetaOption(raw: unknown): LaborStatisticsFilterOption | null {
    if (raw == null || typeof raw !== 'object')
        return null;
    const o = raw as Record<string, unknown>;
    const id = o.id != null ? String(o.id).trim() : '';
...
function metaList(o: Record<string, unknown>, snake: string, camel: string): LaborStatisticsFilterOption[] {
...
function strField(o: Record<string, unknown>, snake: string, camel?: string): string {
    const v = o[snake] ?? (camel ? o[camel] : undefined);
    return v != null ? String(v).trim() : '';
}

function normalizeLaborStatisticsDetailRow(raw: unknown): LaborStatisticsDetailRowApi | null {
    if (raw == null || typeof raw !== 'object')
        return null;
    const o = raw as Record<string, unknown>;
...
function normalizeLaborStatisticsStackedChartRow(raw: unknown): LaborStatisticsStackedChartRow | null {
    if (raw == null || typeof raw !== 'object')
        return null;
    const o = raw as Record<string, unknown>;
...
function chartStackedList(chartsRaw: Record<string, unknown>, snake: string, camel: string): LaborStatisticsStackedChartRow[] {
...
function normalizeTeamFinanceList(chartsRaw: Record<string, unknown>): LaborStatisticsTeamFinanceRow[] {
...
function laborStatisticsQueryParams(q: LaborStatisticsQuery): URLSearchParams {
...
const LABOR_DETAIL_PAGE_SIZE = 200;
const LABOR_DETAIL_MAX_PAGES = 50;
...
function filterActiveProjectsForExpenses(rows: TimeTrackingProjectForExpense[], includeArchived?: boolean): TimeTrackingProjectForExpense[] {
    if (includeArchived)
        return rows;
...
function isTimeTrackingUnavailableError(e: unknown): boolean {
    return e instanceof Error && /503|unavailable|недоступ/i.test(e.message);
}

async function listAllClientProjectsMergedFallback(includeArchived: boolean): Promise<TimeManagerClientProjectRow[]> {
...
const PROJECT_BUDGET_METRICS_CHUNK = 40;
...
function parsePartnerConfirmedReportCommentList(raw: unknown): PartnerConfirmedReportComment[] {
    if (!Array.isArray(raw))
        return [];
...
function parseReportTotals(raw: unknown): ReportTotals | null {
...
async function resolveReportExportDownloadFilename(
