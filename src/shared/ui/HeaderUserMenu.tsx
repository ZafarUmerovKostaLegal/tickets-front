...
function shortDisplayName(displayName: string | null | undefined, email: string | undefined, fallbackUser: string): string {
    if (!displayName?.trim())
...
export type HeaderUserMenuProps = {
    variant?: 'default' | 'inCard' | 'standalone';
};
...
export function HeaderUserMenu({ variant = 'default' }: HeaderUserMenuProps) {
    ...
    return (<div className={rootClass} ref={rootRef}>
      <button type="button" className="header-user-menu__trigger" onClick={onToggle} aria-expanded={open} aria-haspopup="menu" aria-label={t('header.userMenu')} id={triggerId}>
        <span className="header-user-menu__avatar-wrap" aria-hidden>
          {user?.picture && !loading
            ? (<img src={user.picture} className="header-user-menu__avatar" alt="" width={32} height={32}/>)
            : user && !loading
      ...
      {open && (
        <div className="header-user-menu__dropdown" role="menu" aria-labelledby={triggerId}>
          <LanguageSwitcher />
          <div className="header-user-menu__dropdown-sep" role="separator" />
          <button
            type="button"
