...
export function AttendanceDatePicker({ value, onChange }: AttendanceDatePickerProps) {
    ...
    const weekdays = getMessages(locale).attendancePage.datePicker.weekdays;
    ...
    return (
      <div className="att-dp" ref={ref}>
        <button type="button" className="att-dp__trigger" onClick={() => setOpen((v) => !v)}>
          <span className="att-dp__val">{value || t('attendancePage.datePicker.placeholder')}</span>
        ...
        <div className={`att-dp__drop${open ? ' att-dp__drop--open' : ''}`}>
          <div className="att-dp__nav">
            <button type="button" onClick={() => goMonth(-1)} aria-label={t('attendancePage.datePicker.prevMonth')}>‹</button>
            <span>{new Date(year, month, 1).toLocaleDateString(monthLocale, { month: 'long', year: 'numeric' })}</span>
            <button type="button" onClick={() => goMonth(1)} aria-label={t('attendancePage.datePicker.nextMonth')}>›</button>
          </div>
