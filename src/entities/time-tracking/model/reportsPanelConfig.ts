...
export type TimeGroup = 'clients' | 'projects' | 'tasks' | 'team';

export type ExpenseGroup = 'clients' | 'projects' | 'categories' | 'team';
export type GroupByV2 = TimeGroup | ExpenseGroup;
export type PeriodGranularity = 'week' | 'month' | 'quarter' | 'year' | 'all';

export type ReportsSection = 'build' | 'weekly' | 'monthly' | 'partner-confirmed' | 'monthly-archive' | 'for-review';

export type PartnerConfirmedSubview = 'list' | 'archive';
...
export type ReportsPrefsStored = {
    ...
    partnerConfirmedSubview?: PartnerConfirmedSubview;
};
...
export function normalizeReportsSection(section: ReportsSection): ReportsSection {
    return section === 'monthly-archive' ? 'partner-confirmed' : section;
}

export function isPartnerConfirmedSubview(x: unknown): x is PartnerConfirmedSubview {
    return x === 'list' || x === 'archive';
}

export function isReportsSection(x: unknown): x is ReportsSection {
    return x === 'build'
        || x === 'weekly'
        || x === 'monthly'
        || x === 'partner-confirmed'
