...
import type { Comment, PriorityItem, StatusItem, Ticket, TicketsParams, } from './model/types';
...
const _statusesCache = createQueryCache<StatusItem[]>({
    ttlMs: TICKET_STATIC_TTL_MS,
    storageKey: 'statuses',
});
const _prioritiesCache = createQueryCache<PriorityItem[]>({
    ttlMs: TICKET_STATIC_TTL_MS,
    storageKey: 'priorities',
});

export function invalidateTicketStaticCache(): void {
...
export type UpdateTicketData = Partial<Pick<Ticket, 'theme' | 'description' | 'attachment_path' | 'status' | 'category' | 'priority'>> & {
