...
export type TtTimerStorageChangedDetail = {
    authUserId: number;
};
export function notifyTimesheetTimerStorageChanged(authUserId: number): void {
    if (typeof window === 'undefined')
        return;
    window.dispatchEvent(new CustomEvent<TtTimerStorageChangedDetail>(TT_TIMER_STORAGE_CHANGED_EVENT, {
        detail: { authUserId },
    }));
}
